(ns leathergoods.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (is (= 15.0 (:claimed-parts-cost (store/ticket s "ticket-1"))))
      (is (true? (:safety-test-passed? (store/ticket s "ticket-1"))))
      (is (false? (:involves-branded-item? (store/ticket s "ticket-1"))))
      (is (= 60.0 (:claimed-parts-cost (store/ticket s "ticket-3"))))
      (is (false? (:safety-test-passed? (store/ticket s "ticket-4"))))
      (is (true? (:involves-branded-item? (store/ticket s "ticket-5"))))
      (is (false? (:brand-authenticity-confirmed? (store/ticket s "ticket-5"))))
      ...
      (is (nil? (store/brand-screening-of s "ticket-1")))
  ...
  (doseq [[label s] (backends)]
    (testing label
      (testing "partial upsert merges, preserving untouched fields"
        ...
        (is (= "Sakura Tanaka" (:customer (store/ticket s "ticket-1"))))
        (is (= 15.0 (:claimed-parts-cost (store/ticket s "ticket-1"))) "unrelated field preserved"))
      (testing "assessment / safety-screening / brand-screening payloads commit and read back"
        ...
        (store/commit-record! s {:effect :brand-screening/set :path ["ticket-1"]
                                 :payload {:ticket-id "ticket-1" :verdict :not-applicable}})
        (is (= {:ticket-id "ticket-1" :verdict :not-applicable} (store/brand-screening-of s "ticket-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-completion-sequence s "JPN")))
    (is (zero? (store/next-return-sequence s "JPN")))
    (store/with-tickets s {"x" {:id "x" :customer "n" :item "d" :item-type :boots
