(ns leathergoods.governor-contract-test
  "The governor contract as executable tests -- the footwear-and-
  leather-goods-repair analog of `cloud-itonami-isic-9521`'s
  ...
  test`), and structurally closest to `cloud-itonami-isic-9529`'s
  `specialtyrepair.governor-contract-test`. The single invariant under
  test:
...
(deftest parts-cost-mismatch-is-held
  (testing "a claimed parts cost that doesn't equal quantity x unit-price -> HOLD (honest reuse of repairshop/9521's, commrepair/9512's, applianceshop/9522's, furniture/9524's and specialtyrepair/9529's own check)"
    (let [[db actor] (fresh)
...
(deftest safety-test-not-passed-is-held-and-unoverridable
  (testing "a failed post-repair safety test on a ticket -> HOLD, and never reaches request-approval (honest reuse of repairshop/9521's, commrepair/9512's, applianceshop/9522's, furniture/9524's and specialtyrepair/9529's own check)"
    (let [[db actor] (fresh)
...
(deftest brand-authenticity-unconfirmed-is-held-and-unoverridable
  (testing "an unconfirmed brand-authenticity status on a branded ticket -> HOLD, and never reaches request-approval -- exercised via :brand/screen DIRECTLY, not via an actuation op against an unscreened ticket -- the genuinely NEW check this vertical adds, the 69th unconditional-evaluation-discipline grounding overall and the SIXTH conditional variant (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / eldercare's, museum's, conservation's, salon's, entertainment's, casework's, hospital's, facility's, school's, association's, leasing's, behavioral's, secondary's, card's, water's, telecom's, aerospace's, recovery's, consulting's, union's, congregation's, fab's, energy's, care's, navigator's, learning's, banking's, advertising's, polling's, research's, design's, nursing's, sports's, alliedhealth's, laundry's, holdco's, photo's, personalservice's, edsupport's, headoffice's, residential's, cultural's, reserve's, proserv's, sportsevent's, recreation's, sportsclub's, partyops's, memberorg's, commrepair's, applianceshop's, socialresearch's, bizassoc's, vocational's, training's, furniture's and specialtyrepair's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :brand/screen :subject "ticket-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:brand-authenticity-unconfirmed} (-> (store/ledger db) first :basis)))
      (is (nil? (store/brand-screening-of db "ticket-5")) "no clearance written"))))

(deftest brand-screen-is-a-noop-when-not-a-branded-item
  (testing "the brand-authenticity check is CONDITIONAL: a repair that does not involve a branded item has no brand-authenticity requirement at all"
    (let [[_db actor] (fresh)
          res (exec-op actor "t7b" {:op :brand/screen :subject "ticket-1"} operator)]
