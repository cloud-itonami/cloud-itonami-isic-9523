(ns leathergoods.registry
  "Pure-function repair-completion + item-return record construction --
  an append-only leather-goods-repair-shop book-of-record draft,
  ...
  honest, non-fabricating discipline `leathergoods.facts` uses.
  ...
  EXACT-MATCH independent-recompute shape, the SAME real-world concern
  (a claimed parts cost must equal parts-quantity x parts-unit-price)
  reapplied to footwear/leather-goods-repair tickets (soles, heels,
  leather patches, hardware, zippers) rather than consumer-
  electronics, communication-equipment, household-appliance, furniture
  or watch/jewelry/bicycle-repair tickets -- not claimed as new.
  ...
  an item itself (that is `leathergoods.operation`'s `:repair/
...
(defn parts-cost-matches-claim?
  ...
  is an honest, literal reuse of `specialtyrepair.registry`'s own
  check, not a new concept."
...
(defn register-repair-completion
  ...
  the RECORD a shop would keep. `leathergoods.governor` independently
...
(defn register-item-return
  ...
  it builds the RECORD a shop would keep. `leathergoods.governor`
  independently re-verifies the ticket's own post-repair safety-test
  status and brand-authenticity status (when applicable), and blocks a
