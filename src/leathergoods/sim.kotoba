(ns leathergoods.sim
  ...
  brand-authenticity screening -> repair completion (escalate/approve/
  ...
  (screened directly via `:safety/screen`), an unconfirmed brand-
  authenticity status on a branded ticket (screened directly via
  `:brand/screen` -- never via an actuation op against an unscreened
  ticket -- see this actor's own governor ns docstring / the lesson
  ...
  `furniture`'s and `specialtyrepair`'s ADR-0001s already recorded), a
  double completion, and a double return."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== ticket/intake ticket-1 (JPN, clean; not a branded item) ==")
    ...
    (println "== brand/screen ticket-1 (not a branded item; escalates -- human approves) ==")
    (println (exec-op actor "t3b" {:op :brand/screen :subject "ticket-1"} operator))
    ...
    (println "== repair/complete ticket-3 (claimed 60.0 vs recompute 40.0 -> HARD hold) ==")
    ...
    (println "== brand/screen ticket-5 (branded item, authenticity unconfirmed -> HARD hold, never reaches a human) ==")
    (println (exec-op actor "t10" {:op :brand/screen :subject "ticket-5"} operator))
