(ns leathergoods.store
  "SSoT for the footwear-and-leather-goods-repair actor, behind a
  ...
  closely modeled on `cloud-itonami-isic-9529`'s `specialtyrepair.
  store`:
  ...
  (test/leathergoods/store_contract_test.clj), which is the whole
  ...
  store`'s/`furniture.store`'s/`specialtyrepair.store`'s own dual
  history, this actor has TWO actuation events (repair completion,
  item return) acting on the SAME entity (a repair ticket), each with
  its OWN history collection, sequence counter and dedicated
  double-actuation-guard boolean (`:repair-completed?`/
  `:item-returned?`, never a `:status` value).

  Beyond `specialtyrepair.store`'s own `hallmark-screening-of`, this
  store carries `brand-screening-of` (brand-authenticity/anti-
  counterfeiting confirmation status) -- the genuinely new concern
  this vertical adds, since repairing a branded/trademarked item (one
  of this blueprint's own named example activities -- designer
  handbags, branded footwear) is a real, distinct regulatory concern
  beyond `repairshop`/9521's, `commrepair`/9512's, `applianceshop`/
  9522's, `furniture`/9524's and `specialtyrepair`/9529's own
  catalogs.
  ...
  brand-authenticity status, which repair was completed, which item
...
(defprotocol Store
  ...
  (brand-screening-of [s ticket-id] "committed brand-authenticity screening verdict for a ticket, or nil")
  (assessment-of [s ticket-id] "committed jurisdiction assessment, or nil")
  (ledger [s])
  (completion-history [s] "the append-only repair-completion history (leathergoods.registry drafts)")
  (return-history [s] "the append-only item-return history (leathergoods.registry drafts)")
...
(defn demo-data
  ...
  []
  {:tickets
   {"ticket-1" {:id "ticket-1" :customer "Sakura Tanaka" :item "Leather boots (sole resoling)" :item-type :boots
                 :parts-quantity 1 :parts-unit-price 15 :claimed-parts-cost 15.0
                 :safety-test-passed? true
                 :involves-branded-item? false :brand-authenticity-confirmed? false
                 :repair-completed? false :item-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-2" {:id "ticket-2" :customer "Atlantis Doe" :item "Leather belt (buckle replacement)" :item-type :belt
                 :parts-quantity 1 :parts-unit-price 10 :claimed-parts-cost 10.0
                 :safety-test-passed? true
                 :involves-branded-item? false :brand-authenticity-confirmed? false
                 :repair-completed? false :item-returned? false
                 :jurisdiction "ATL" :status :intake}
    "ticket-3" {:id "ticket-3" :customer "鈴木一郎" :item "Leather handbag (strap restitching)" :item-type :handbag
                 :parts-quantity 1 :parts-unit-price 40 :claimed-parts-cost 60.0
                 :safety-test-passed? true
                 :involves-branded-item? false :brand-authenticity-confirmed? false
                 :repair-completed? false :item-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-4" {:id "ticket-4" :customer "田中花子" :item "Leather shoes (heel replacement)" :item-type :shoes
                 :parts-quantity 1 :parts-unit-price 20 :claimed-parts-cost 20.0
                 :safety-test-passed? false
                 :involves-branded-item? false :brand-authenticity-confirmed? false
                 :repair-completed? false :item-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-5" {:id "ticket-5" :customer "佐藤次郎" :item "Designer handbag (hardware/clasp repair)" :item-type :handbag
                 :parts-quantity 1 :parts-unit-price 35 :claimed-parts-cost 35.0
                 :safety-test-passed? true
                 :involves-branded-item? true :brand-authenticity-confirmed? false
                 :repair-completed? false :item-returned? false
                 :jurisdiction "JPN" :status :intake}}})
...
(defrecord MemStore [a]
  ...
  (brand-screening-of [_ id] (get-in @a [:brand-screenings id]))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :brand-screening/set
      (swap! a assoc-in [:brand-screenings (first path)] payload)
...
(defn seed-db
  ...
  (->MemStore (atom (assoc (demo-data)
                           :assessments {} :safety-screenings {} :brand-screenings {}
...
(def ^:private schema
  ...
  Map/compound values (assessment/safety-screening/brand-screening
  ...
  {:ticket/id                  {:db/unique :db.unique/identity}
   ...
   :brand-screening/ticket-id  {:db/unique :db.unique/identity}
...
(defn- ticket->tx [{:keys [id customer item item-type parts-quantity parts-unit-price claimed-parts-cost
                          safety-test-passed? involves-branded-item? brand-authenticity-confirmed?
  ...
  (cond-> {:ticket/id id}
    ...
    (some? involves-branded-item?)                               (assoc :ticket/involves-branded-item? involves-branded-item?)
    (some? brand-authenticity-confirmed?)                          (assoc :ticket/brand-authenticity-confirmed? brand-authenticity-confirmed?)
...
(def ^:private ticket-pull
  [:ticket/id :ticket/customer :ticket/item :ticket/item-type :ticket/parts-quantity
   ...
   :ticket/involves-branded-item? :ticket/brand-authenticity-confirmed?
...
(defn- pull->ticket [m]
  (when (:ticket/id m)
    {:id (:ticket/id m) :customer (:ticket/customer m) :item (:ticket/item m)
     ...
     :involves-branded-item? (boolean (:ticket/involves-branded-item? m))
     :brand-authenticity-confirmed? (boolean (:ticket/brand-authenticity-confirmed? m))
...
(defrecord DatomicStore [conn]
  ...
  (safety-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
              ...
              (d/db conn) id)))
  (brand-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
                :where [?k :brand-screening/ticket-id ?tid] [?k :brand-screening/payload ?p]]
              (d/db conn) id)))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :brand-screening/set
      (d/transact! conn [{:brand-screening/ticket-id (first path) :brand-screening/payload (enc payload)}])
