(ns leathergoods.repairopsllm
  "RepairOps-LLM client -- the *contained intelligence node* for the
  footwear-and-leather-goods-repair actor.

  It normalizes ticket intake, drafts a per-jurisdiction consumer-
  product-safety/brand-authenticity evidence checklist, screens
  tickets for a failed post-repair safety test, screens tickets for an
  unconfirmed brand-authenticity status, drafts the repair-completion
  ...
  `leathergoods.governor` before anything touches the SSoT, and
...
(defn- assess-jurisdiction
  "Per-jurisdiction consumer-product-safety/brand-authenticity
  ...
  NO official spec-basis in `leathergoods.facts` -- the Repair Shop
  ...
  (let [t (store/ticket db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式spec-basisが見つかりません")
       :rationale  "leathergoods.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
...
(defn- screen-brand
  "Brand-authenticity screening draft -- the genuinely new screening
  concern this vertical adds. `:brand-authenticity-confirmed? false`
  on a ticket that itself involves a branded item injects the failure
  mode: the Repair Shop Governor must HOLD, un-overridably, on any
  unconfirmed brand-authenticity status."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)]
    (cond
      (nil? t)
      {:summary "対象ticketが見つかりません" :rationale "no ticket record"
       :cites [] :effect :brand-screening/set :value {:ticket-id subject :verdict :unknown}
       :stake nil :confidence 0.0}

      (not (true? (:involves-branded-item? t)))
      {:summary    (str (:customer t) ": ブランド品ではない -- 真贋確認は不要")
       :rationale  "involves-branded-item? が false のため、ブランド真贋確認要件そのものが発生しない。"
       :cites      [:brand-determination]
       :effect     :brand-screening/set
       :value      {:ticket-id subject :verdict :not-applicable}
       :stake      nil
       :confidence 0.9}

      (not (true? (:brand-authenticity-confirmed? t)))
      {:summary    (str (:customer t) ": ブランド真贋確認が未完了")
       :rationale  "ブランド品だが真贋確認が未完了。人手確認とホールドが必須。"
       :cites      [:brand-check]
       :effect     :brand-screening/set
      ...
      :else
      {:summary    (str (:customer t) ": ブランド真贋確認済み")
       :rationale  "ブランド品、真贋確認済み。"
       :cites      [:brand-check]
       :effect     :brand-screening/set
...
(defn- propose-repair-completion
  ...
  a phase's `:auto` set (`leathergoods.phase`); the governor also
...
(defn- propose-item-return
  ...
  REAL-WORLD act (an unsafe item, or a branded item without confirmed
  authenticity, could reach a customer), never a draft the actor may
  auto-run. See README `Actuation`: no phase ever adds this op to a
  phase's `:auto` set (`leathergoods.phase`); the governor also always
  escalates on `:actuation/return-item`. Two independent layers agree,
  deliberately."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)
        ready? (and t (:safety-test-passed? t)
                   (or (not (:involves-branded-item? t))
                       (:brand-authenticity-confirmed? t)))]
    {:summary    (str subject " 向け返却提案"
                      (when t (str " (customer=" (:customer t) ")")))
     :rationale  (if t
                   (str "safety-test-passed?=" (:safety-test-passed? t)
                        " involves-branded-item?=" (:involves-branded-item? t)
                        " brand-authenticity-confirmed?=" (:brand-authenticity-confirmed? t))
                   "ticketが見つかりません")
     :cites      (if t [subject] [])
...
(defn infer
  ...
  (case op
    ...
    :brand/screen                           (screen-brand db request)
