...
`test/leathergoods/phase_test.kotoba`'s `repair-complete-never-auto-at-
...
| File | Role |
|---|---|
| `src/leathergoods/store.kotoba` | **Store** protocol -- `MemStore` ‖ `DatomicStore` (`langchain.db`) + append-only audit ledger + repair-completion AND item-return history (dual history, mirroring `repairshop`/9521's, `commrepair`/9512's, `applianceshop`/9522's, `furniture`/9524's and `specialtyrepair`/9529's own shape). The double-actuation guard checks dedicated `:repair-completed?`/`:item-returned?` booleans rather than a `:status` value |
| `src/leathergoods/registry.kotoba` | Repair-completion/item-return draft records, plus `parts-cost-matches-claim?` -- an HONEST, literal reuse of `repairshop.registry`'s/`commrepair.registry`'s/`applianceshop.registry`'s/`furniture.registry`'s/`specialtyrepair.registry`'s own EXACT-MATCH independent-recompute check for the SAME real-world concern, not claimed as new |
| `src/leathergoods/facts.kotoba` | Per-jurisdiction consumer-product-safety AND brand-authenticity/anti-counterfeiting catalog (a genuine extension beyond `repairshop.facts`'s, `commrepair.facts`'s, `applianceshop.facts`'s, `furniture.facts`'s and `specialtyrepair.facts`'s own catalogs) with an official spec-basis citation per entry, honest coverage reporting -- ALL FOUR seeded jurisdictions have a brand-authenticity sub-citation here, unlike prior siblings' own honest single-jurisdiction gap |
| `src/leathergoods/repairopsllm.kotoba` | **RepairOps-LLM** -- `mock-advisor` ‖ `llm-advisor`; intake/jurisdiction-assessment/safety-screening/brand-screening/repair-completion/item-return proposals |
| `src/leathergoods/governor.kotoba` | **Repair Shop Governor** -- 7 HARD checks (spec-basis · evidence-incomplete · parts-cost-mismatch, honest reuse · safety-test-not-passed, honest reuse · brand-authenticity-unconfirmed, CONDITIONAL unconditional evaluation, GENUINELY NEW, the 69th grounding of this discipline · already-completed guard · already-returned guard) + 1 soft (confidence/actuation gate) |
| `src/leathergoods/phase.kotoba` | **Phase 0→3** -- read-only → assisted intake → assisted assess → supervised (repair completion/item return always human; ticket intake is the ONLY auto-eligible op, no direct capital risk) |
| `src/leathergoods/operation.kotoba` | **OperationActor** -- langgraph StateGraph |
| `src/leathergoods/sim.kotoba` | demo driver |
